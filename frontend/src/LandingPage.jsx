...
import ChatroomIdList from "./components/ChatroomIdList";
      ...
      <Card className="max-w-screen max-h-screen h-screen border-none rounded-none bg-zinc-700 ">
        <CardContent className="flex flex-col items-center justify-center mt-24">
          ...
          <Input
            ...
            className="bg-[#5d5d5d] max-w-50 border-none text-white placeholder:text-neutral-400 mt-8"
            onChange={(e) => setUsername(e.target.value)}
            value={username ?? ""}
          />
          <Input
            placeholder="ChatRoom ID:"
            className="bg-[#5d5d5d] max-w-50 border-none mt-3 text-white placeholder:text-neutral-400"
            onChange={(e) => setChatroomId(e.target.value)}
            value={chatroomId ?? ""}
          />

          <Button
            className="bg-[#3f463f] mt-6"
            onClick={() => {
